    ...
    <div className={"quote alert alert-light"}>
      <div className={"d-flex align-items-center justify-content-between"}>
        <div className={"text-secondary"} style={{fontSize:12}}>
          <strong className={"text-primary border-bottom border-primary"}>Created on:</strong> {format(quote.createdAt, "dd.MM.yyyy HH:mm")}
        </div>
        <div className={"d-flex flex-row gap-1"}>
          <Link to={"/delete"} onClick={() => deleteQuote(quote.id)} className={"btn btn-primary"} style={deleteBtn}></Link>
      ...
      <div>
        ...
        <div className={"text-break text-end mt-2"}><strong className={"text-primary"}>Author:</strong> ____{quote.author}</div>
      </div>
    </div>
  );
};
